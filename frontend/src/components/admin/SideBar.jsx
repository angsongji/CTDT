...
function SideBar() {
    const Component = ({ chucnang }) => (
        <NavLink
            ...
            className={({ isActive }) =>
                `transition-all duration-200  flex items-center gap-3 p-2 w-full font-bold pl-4 ${
                    isActive ? "active text-white" : " hover:border-r-5 hover:border-[var(--light-pink)] text-[var(--medium-pink)]"
                }`
            }
        >
            {/* Đổi màu icon dựa vào isActive */}
            {({ isActive }) => (
                <>
